...
- **Movie pages can be multi-model magazine collections** — magazine-scan movie pages (e.g. SoHo, H Para Hombres) contain images of MANY models, not just the target. Each image is captioned with its model name via `lightbox="Magazine<small>{Model}</small>"` — parse these and keep only images whose caption matches the target's known name. Magazine images are often already in the celeb-page download, so expect heavy overlap — dedupe by URL.
