...
- **PornPics** (pornpics.com): Largest porn star photo gallery site. 15-21 images per gallery. Use `gallery-dl` for bulk downloads: `gallery-dl "https://www.pornpics.com/pornstars/{name}/"` (see skill `find-media-from-pornpics`). Search for extras: `gallery-dl "https://www.pornpics.com/?q={name}+facial"`. Full-size images ~2MB each. **NOTE**: The direct model page URL `pornstars/{name}/` may only download 1 image per gallery for some models. When this happens, use the query search `https://www.pornpics.com/?q={name}` instead to get full galleries with all images. Images served from `cdni.pornpics.com` CDN — `460/` path is thumbnail (460px), `1280/` would be full-size. The single gallery index page (`/galleries/{slug}/`) contains ALL photo thumbnails at the `460/` path — derive full-size URLs by replacing `/460/` with `/1280/` and download with `curl -H "User-Agent: Mozilla/5.0"` (1s delay). **NOTE**: `gallery-dl` may hang on PornPics (0-byte `.part` file, no progress) — when that happens fall back to the manual curl extraction above. The search results page (`/?q={name}+facial`) lists gallery slugs with titles, so you can pick only facial/cumshot-relevant galleries.
- **MyPornstarBook** (mypornstarbook.net): 45+ galleries per star, ~16 images each. Thumbnails accessible at `/thumbnails/tn{N}.jpg` — full images blocked by Cloudflare (403). Pattern: `https://www.mypornstarbook.net/pornstars/{first_letter}/{name}/gallery{NN}/thumbnails/tn{NN}.jpg` (note: the `src` attribute in gallery HTML is the *relative* `images/thumbnails/tnNN.jpg` — prepend the `{first_letter}/{name}/gallery{NN}/` path, not `images/`). **NOTE**: gallery-dl does NOT support MyPornstarBook. Manual extraction using `curl -H "User-Agent: Mozilla/5.0"` required. The star index page (`/pornstars/{l}/{name}/index.php`) contains all gallery titles inline (`<a href="...galleryNN/index.php">Title</a>`) — decode as latin-1 and grep title text (e.g. `facial`, `cum`) to find facial/cumshot galleries.
- **OK.Porn** (ok.porn): Photo set albums (studio photo sets, often facial/cumshot sets). Album page at `ok.porn/albums/{id}/`. Image URLs in page HTML: `https://pics-storage-1.ok.porn/contents/albums/main/1920x1080/{id/1000}/{id}/{imgId}.jpg` (largest available size). No Cloudflare — plain `curl -H "User-Agent: Mozilla/5.0"` works. gallery-dl does NOT support ok.porn. Album titles in page `<title>` reveal content (e.g. "Cum On My Face").
...
- **Pichunter** (pichunter.com): Gallery site with model pages at `/models/{name_underscored}`. **NOTE**: Full-size images (`post_big.jpg`) return 403 Forbidden — only thumbnails available. Image URLs: `https://cdn{NNNN}.pichunter.com/media/posts/12--{postId}/conversions/{num}-{hash}-post_archive_thumb.jpg`. Multiple images may exist per gallery. Gallery-dl does NOT support Pichunter — manual extraction required. Requires User-Agent header. Images served from `cdn{NNNN}.pichunter.com`.
...
- **Pichunter thumb-only**: `post_big.jpg` URLs on Pichunter return 403 Forbidden — only thumbnail images (`post_archive_thumb.jpg`) are accessible. Download thumbnails as-is.