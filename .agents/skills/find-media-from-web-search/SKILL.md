...
- **Babepedia** (babepedia.com): Profile pages with user-uploaded photos and biographical data. Images at `/pics/{Slug}.jpg` (main gallery), `/pics/{Slug}N.jpg` (additional). Also has `/user-uploads/` directory for community uploads (`/user-uploads/{BabeName}.jpg` through `{BabeName}15.jpg`). Useful for aliases, biographical data, and gallery links. Has multiple galleries with additional galleries available. **NOTE**: Returns 403 Forbidden for direct `curl`/`urllib` requests due to Cloudflare — use `web_fetch` with markdown format instead. Gallery pages at `/gallery/{Slug}/{id}`. User uploads may contain images not found in the main gallery.
...
- **Pichunter** (pichunter.com): Gallery site with model pages at `/models/{name_underscored}`. Full-size images use `post_single_big.jpg` extension in the CDN path. Extract URLs with: `curl -sL "$URL" | grep -oE 'post_single_big\.jpg'`. Multiple images may exist per gallery. Requires User-Agent header. Images served from `cdn{NNNN}.pichunter.com`.
