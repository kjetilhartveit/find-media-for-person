...
- Image URL shape: `https://fapeza.com/media/{x}/{y}/{slug}/{folder}/{filename}` where:
  - **`{x}/{y}` is NOT fixed** — it varies per profile (seen: `k/y`, `l/e`, `l/a`). Guessing it produces 404s.
  - `folder = floor(post_id/1000)*1000 + 1000` (e.g., ID 1011→folder 2000, ID 5826→folder 6000, ID 29→folder 1000)
  - `filename`: For IDs >= 1000 → `{slug}_{id}.jpg`; For IDs < 1000 → zero-padded to 4 digits: `{slug}_{id:04d}.jpg` (e.g., `kylie-jenner_0029.jpg`)
  - The Referer header is NOT required for image requests.
- **Recommendation:** Do NOT construct URLs blindly — extract the real image URLs from profile/post page source (`https://fapeza.com/media/[a-z]/[a-z]+/{slug}/...`), stripping `_400px.` for full size. Construction with 404-skipping only works when the prefix is already known from a loaded page.
...
- Profile pages use `https://fapeza.com/{slug}/page-N/` for pagination. It can be fully functional (e.g., a 116-post profile paginated across 7 pages) or broken (404s, or cycling back to page 1 content).
- Page 1 (no `/page-N/`) shows the most recent posts.
- **Pagination loop:** fetch pages sequentially until a page's post-ID set is a subset of already-seen IDs (two repeats in a row = cycling, stop). Do not rely on `/page-N/` returning 200 as a stop signal.
- If pagination works, each page shows ~19-20 posts.
...
- No special headers (Referer etc.) are required for image requests — plain GET works via cloudscraper.
...
- Some models simply do not have content on Fapeza. For example, "Megan Vale" search returned 0 results. Always verify by searching the site first (POST `q=NAME` to `fapeza.com/search/`) before assuming a profile won't exist.
...
- **Videos exist on some profiles** — a post page can reference `{slug}_{id}.mp4` next to its `.jpg` thumbnail on the same media path. Check post pages for `.mp4` URLs before assuming image-only.
- **Alias lists can conflate different people with similar names.** A profile's alias list in the page title may mix handles of similar-but-different people. Verify identity by cross-checking alias handles against the target person's KNOWN social handles (e.g., actual OnlyFans/Instagram usernames) via a web search, and confirm the search-result post count matches the enumerated posts before trusting a slug.
