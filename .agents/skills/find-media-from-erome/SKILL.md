...
- Filter out different people with similar names: searching for "Megan Vale" returned albums of "Megan Blaze" (a different adult actress) from users like `Cartero` (titles like "VALE CASTA Y MEGAN BLAZE" — "vale" is Spanish for "valley") and `fukumeankrv` (titles like "megan_blaze01/vale_loveyou12"). Always verify album titles contain the full name "Megan Vale" not just parts of it.
